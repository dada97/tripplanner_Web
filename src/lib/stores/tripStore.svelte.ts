...
class TripStore {
          ...
          parsedTrips.forEach((trip: Trip) => {
            ...
            trip.days.forEach((day: any) => {
              ...
              if (!day.journals) {
                ...
                if (
                  day.journal &&
                  (day.journal.content || day.journal.location)
                ) {
                  day.journals.push({
                    id: generateId(),
  ...
  addTrip(trip: Trip) {
    ...
    if (!trip.generalExpenses) trip.generalExpenses = [];
  ...
  addJournalEntry(tripId: string, dayIndex: number, entry: JournalEntry) {
    ...
    if (trip && trip.days[dayIndex]) {
      ...
      // Sort by timestamp
      trip.days[dayIndex].journals.sort((a, b) =>
  ...
  updateJournalEntry(tripId: string, dayIndex: number, entry: JournalEntry) {
    const trip = this.getTrip(tripId);
    if (trip && trip.days[dayIndex]) {
      const index = trip.days[dayIndex].journals.findIndex(
        (j) => j.id === entry.id
      );
      if (index !== -1) {
  ...
  removeJournalEntry(tripId: string, dayIndex: number, entryId: string) {
    const trip = this.getTrip(tripId);
    if (trip && trip.days[dayIndex]) {
      trip.days[dayIndex].journals = trip.days[dayIndex].journals.filter(
        (j) => j.id !== entryId
      );
    }
  }
  ...
  updateActivity(tripId: string, dayIndex: number, activity: Activity) {
    const trip = this.getTrip(tripId);
    if (trip && trip.days[dayIndex]) {
      const index = trip.days[dayIndex].activities.findIndex(
        (a) => a.id === activity.id
      );
      if (index !== -1) {
  ...
  removeActivity(tripId: string, dayIndex: number, activityIndex: number) {
    const trip = this.getTrip(tripId);
    if (trip && trip.days[dayIndex]) {
  ...
  addExpense(
    ...
    const trip = this.getTrip(tripId);
    if (trip) {
      const activity = trip.days[dayIndex]?.activities.find(
        (a) => a.id === activityId
      );
      if (activity) {
  ...
  addGeneralExpense(tripId: string, expense: Expense) {
    const trip = this.getTrip(tripId);
    if (trip) {
      if (!trip.generalExpenses) trip.generalExpenses = [];
      trip.generalExpenses.push(expense);
    }
  }

  updateGeneralExpense(tripId: string, expense: Expense) {
    const trip = this.getTrip(tripId);
    if (trip && trip.generalExpenses) {
      const index = trip.generalExpenses.findIndex((e) => e.id === expense.id);
      if (index !== -1) {
        trip.generalExpenses[index] = expense;
      }
    }
  }

  removeGeneralExpense(tripId: string, expenseId: string) {
    const trip = this.getTrip(tripId);
    if (trip && trip.generalExpenses) {
      trip.generalExpenses = trip.generalExpenses.filter(
        (e) => e.id !== expenseId
      );
    }
  }
